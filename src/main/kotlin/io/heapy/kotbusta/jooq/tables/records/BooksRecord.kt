/*
 * This file is generated by jOOQ.
 */
package io.heapy.kotbusta.jooq.tables.records


import io.heapy.kotbusta.jooq.tables.Books
import io.heapy.kotbusta.jooq.tables.interfaces.IBooks

import java.time.OffsetDateTime

import org.jooq.Record1
import org.jooq.impl.UpdatableRecordImpl


/**
 * This class is generated by jOOQ.
 */
@Suppress("warnings")
open class BooksRecord private constructor() : UpdatableRecordImpl<BooksRecord>(Books.BOOKS), IBooks {

    open override var id: Long
        set(value): Unit = set(0, value)
        get(): Long = get(0) as Long

    open override var title: String
        set(value): Unit = set(1, value)
        get(): String = get(1) as String

    open override var `annotation`: String?
        set(value): Unit = set(2, value)
        get(): String? = get(2) as String?

    open override var genre: String?
        set(value): Unit = set(3, value)
        get(): String? = get(3) as String?

    open override var language: String?
        set(value): Unit = set(4, value)
        get(): String? = get(4) as String?

    open override var seriesId: Long?
        set(value): Unit = set(5, value)
        get(): Long? = get(5) as Long?

    open override var seriesNumber: Int?
        set(value): Unit = set(6, value)
        get(): Int? = get(6) as Int?

    open override var filePath: String
        set(value): Unit = set(7, value)
        get(): String = get(7) as String

    open override var archivePath: String
        set(value): Unit = set(8, value)
        get(): String = get(8) as String

    open override var fileSize: Long?
        set(value): Unit = set(9, value)
        get(): Long? = get(9) as Long?

    open override var dateAdded: OffsetDateTime
        set(value): Unit = set(10, value)
        get(): OffsetDateTime = get(10) as OffsetDateTime

    open override var coverImage: ByteArray?
        set(value): Unit = set(11, value)
        get(): ByteArray? = get(11) as ByteArray?

    open override var createdAt: OffsetDateTime?
        set(value): Unit = set(12, value)
        get(): OffsetDateTime? = get(12) as OffsetDateTime?

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    override fun key(): Record1<Long?> = super.key() as Record1<Long?>

    // -------------------------------------------------------------------------
    // FROM and INTO
    // -------------------------------------------------------------------------

    fun from(from: IBooks) {
        this.id = from.id
        this.title = from.title
        this.`annotation` = from.`annotation`
        this.genre = from.genre
        this.language = from.language
        this.seriesId = from.seriesId
        this.seriesNumber = from.seriesNumber
        this.filePath = from.filePath
        this.archivePath = from.archivePath
        this.fileSize = from.fileSize
        this.dateAdded = from.dateAdded
        this.coverImage = from.coverImage
        this.createdAt = from.createdAt
        resetTouchedOnNotNull()
    }

    /**
     * Create a detached, initialised BooksRecord
     */
    constructor(id: Long, title: String, `annotation`: String? = null, genre: String? = null, language: String? = null, seriesId: Long? = null, seriesNumber: Int? = null, filePath: String, archivePath: String, fileSize: Long? = null, dateAdded: OffsetDateTime, coverImage: ByteArray? = null, createdAt: OffsetDateTime? = null): this() {
        this.id = id
        this.title = title
        this.`annotation` = `annotation`
        this.genre = genre
        this.language = language
        this.seriesId = seriesId
        this.seriesNumber = seriesNumber
        this.filePath = filePath
        this.archivePath = archivePath
        this.fileSize = fileSize
        this.dateAdded = dateAdded
        this.coverImage = coverImage
        this.createdAt = createdAt
        resetTouchedOnNotNull()
    }

    /**
     * Create a detached, initialised BooksRecord
     */
    constructor(value: IBooks?): this() {
        if (value != null) {
            this.id = value.id
            this.title = value.title
            this.`annotation` = value.`annotation`
            this.genre = value.genre
            this.language = value.language
            this.seriesId = value.seriesId
            this.seriesNumber = value.seriesNumber
            this.filePath = value.filePath
            this.archivePath = value.archivePath
            this.fileSize = value.fileSize
            this.dateAdded = value.dateAdded
            this.coverImage = value.coverImage
            this.createdAt = value.createdAt
            resetTouchedOnNotNull()
        }
    }
}
